...
When user asks to search battery papers, run ALL available databases in parallel:

| Database | Tool/Method | Strengths | Weaknesses |
|----------|------------|-----------|------------|
| Google Scholar | `gs-search` | Broad coverage, citation tracking, good for English papers | No structured data extraction |
| CNKI (知网) | `cnki-search` | Chinese papers, theses, patents | Chinese only |
| arXiv | `arxiv` | Preprints, fastest for new results | Not peer-reviewed |
| Web of Science | `WebSearch site:webofscience.com` | Structured metadata, citation reports | Paywalled |
| PubMed | `WebSearch site:pubmed.ncbi.nlm.nih.gov` | Biomedical/bio-battery | Limited for materials |

**Parallel search workflow**:
1. Issue all searches simultaneously
2. Merge results, deduplicate by DOI/title
3. Sort by relevance (citation count + keyword match + journal tier)
4. Report: total found, deduplicated count, top 10 with 1-line summaries

### Publisher-Specific Direct Search

When searching for very specific data (e.g., "Li6PS5Cl ionic conductivity > 10 mS/cm"):

| Publisher | Search URL pattern |
|-----------|-------------------|
| RSC (EES, JMCA, PCCP) | `site:pubs.rsc.org {keywords}` |
| ACS (JACS, Nano Lett, Chem Mater) | `site:pubs.acs.org {keywords}` |
| Wiley (AM, AEM, Angew) | `site:onlinelibrary.wiley.com {keywords}` |
| Elsevier (JPS, Electrochim Acta) | `site:sciencedirect.com {keywords}` |
| Springer Nature (Nat Energy, Nat Mater) | `site:nature.com {keywords}` |

### Search Result Quality Filter

When scanning results, apply quick triage:
- Skip if: conference abstract only, < 2 pages, no experimental data
- Flag if: review in top journal (>500 citations → must-read)
- Prioritize: 2024-2026 papers in AM/AEM/ACS Energy Lett/Angew/EES/Nat Energy
