---
name: research-lab
description: "科研全流程智能调度台(v3.0 吸收第三轮外部skill)。覆盖论文检索(知网/GS/arXiv/BGPT)→系统文献调研(survey)→阅读审稿→写作润色→引文验证→投稿审计→中英翻译→PPT汇报→实验分析→电池材料计算→idea评估→实验设计→科学可视化。说人话即可，自动识别意图路由到对应子skill。触发: 搜论文/下载论文/审稿/写引言/润色/做PPT/评估idea/分析数据/文献综述/文献调研/系统读论文/验证引用/投稿审计/翻译论文/LaTeX写论文/电池/电化学/晶体结构/相图/Materials Project/实验设计。"
metadata:
  version: "3.0.0"
  mega: true
  subskills:
    ...
    paper_reading: [academic-paper, academic-paper-reviewer, pre-submission-reviewer, research-summary, literature-review-writing, academic-pipeline, literature-survey]
    paper_writing: [research-paper-writing, battery-writer, nature-polishing, nature-citation, nature-data, intro-drafter, tech-paper-template, benchmark-paper-template, citation-check]
  ...
  absorbed_from:
    ...
    - literature-survey-skill (SNL-UCSB): intent→triage→deepen→synthesize→expand 结构化文献调研，Kahneman+Keshav+第一性原理
    - citation-check-skill (serenakeyitan): 双轮引用验证，主张提取规则，学术精度数值检查，状态决策树
    - PaperSpine (WUBING2023): 完整性审计(10+检查清单)，中文翻译包(row-by-row)，多智能体独立评审
    - Nature-Paper-Skills (Boom5426): 系统化审稿流程(初始→逐节→综合)，Nature选刊决策，投稿预检清单
    - latex-paper-skills (yunshenwuchuxun): 门控LaTeX管线(issues CSV合同)，引文验证，paper-from-zero路由，results-backfill
---

# 🔬 科研工作台 (research-lab) v3.0

吸收三轮外部 skill 生态后的升级版。你不需要记住任何子 skill 的名字，直接说人话，自动路由。
...
| "做系统文献综述/调研XXX方向" | `literature-survey` → `/survey intent` 引导四模式（探索者/调查者/验证者/考官） |
| "帮我批量筛选论文/画领域全景图" | `literature-survey` → `/survey triage` Pass 1 快速筛选 + 全景图 |
| "深度读这篇论文/提取写作工艺" | `literature-survey` → `/survey deepen` Pass 2/3 + 写作工艺提取 + 可视化分析 |
| "综合这些论文写 Related Work/找 Gap" | `literature-survey` → `/survey synthesize` 跨论文综合分析 |
| "调研语料该扩展哪些论文" | `literature-survey` → `/survey expand` 语料扩展建议 |
...
| "验证引用/查编造引用/核实数据" | `citation-check` → 双轮验证（Pass 1 提取主张 → Pass 2 逐条 Web 验证） |
| "这篇论文的数据有没有编造" | `citation-check` → 学术精度模式（96.555% ≠ 97%） |
| "仅对照原文验证（不搜索网络）" | `citation-check` → Doc-Only 模式 |
| "投稿前全面审计我的论文" | PaperSpine audit → 10+ 完整性检查（artifact/引文质量/LaTeX安全/多Agent评审） |
| "把我的论文翻译成中文" | PaperSpine translate → 完整中文翻译包（row-by-row 表格 + 全文翻译 + 覆盖率验证） |
| "从零开始用LaTeX写论文" | `research-paper-writing` → latex-paper gated workflow（plan→issues CSV→per-issue writing→verify→compile） |
| "回填实验数据到论文草稿" | `research-paper-writing` → results-backfill 模式（placeholder→verified claims） |
| "选Nature还是Nature Methods投稿" | `battery-writer` → Nature Portfolio 选刊决策树 + 文章类型检查 + 投稿预检 |
...
8. **文献调研类** → 优先用 `/survey intent` 开始，确定用户原型后再启动 triage
9. **引文验证类** → 始终双轮（先提取后验证），绝不交叉
...
- `/lab ppt` = 组会 PPT
- `/lab idea` = 评估 idea
- `/lab mat [材料名/结构]` = pymatgen 材料计算
- `/lab design [实验主题]` = 实验设计
- `/lab survey` = 系统文献调研（四模式入口）
- `/lab verify` = 引文验证（双轮）
- `/lab audit` = 投稿前审计
- `/lab translate` = 论文中译
- `/lab latex [主题]` = LaTeX 门控写作
